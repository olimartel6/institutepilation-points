      ...
      <div className="admin-header">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div>
            ...
            <div style={{ fontSize: 11, opacity: 0.5, textTransform: 'uppercase', letterSpacing: 1 }}>Panneau d'administration</div>
          </div>
              ...
              <div className="client-row" onClick={() => handleAddPoints(client)} style={{ cursor: 'pointer' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                    depuis {new Date(client.created_at).toLocaleDateString('fr-CA', { month: 'short', year: 'numeric' })}
                  </div>
                </div>
              </div>
              {addPointsClient?.id === client.id && (
                <div style={{ padding: '16px 0', borderBottom: '1px solid var(--border)', background: 'var(--bg-warm)', margin: '0 -24px', padding: '16px 24px' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button className="btn btn-accent btn-small" style={{ flex: 1 }} onClick={submitPoints}>Ajouter</button>
                    <button className="btn btn-secondary btn-small" style={{ flex: 1 }} onClick={() => setAddPointsClient(null)}>Annuler</button>
                  </div>
                </div>
              )}
            </div>
